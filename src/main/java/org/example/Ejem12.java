...
public class Ejem12 {
    ...
    public static void main(String[] args) {
        ...
        //con el metodo contains se valida si el registro existe en la lista
        System.out.println("******Valida si existe el registro Pedro Picapiedra***********");
        boolean validaExistencia = listaAlumnos2.contains("Pedro Picapiedra");
        System.out.println(validaExistencia);


        System.out.println("****** Mostrar el registro con el mayor número de caracteres ***********");
        String maxString = listaAlumnos.stream().max(Comparator.comparingInt(String::length)).get();
        System.out.println(maxString +" --Total de caracteres: "+ maxString.length() );

        //eliminar un objeto de la lista con lamda
        listaAlumnos.removeIf(alumno -> "Pedro Picapiedra".equalsIgnoreCase(alumno));

        System.out.println("******Se recorre lista se nota que se eliminó un registro***********");
        for(String alumno: listaAlumnos){
            System.out.println(alumno);
